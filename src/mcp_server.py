...
def load_districts():
    try:
        data = cache.setdefault("districts", requests.get(DISTRICTS_URL,
                                                         timeout=10).json())
        ...
        for item in data["data"]:
            ...
            city_mapping[c] = name
        logging.info(f"Carregados {len(city_mapping)} cidades e "
...
def load_weather_types():
    try:
        data = cache.setdefault("weather_types",
                               requests.get(WEATHER_TYPES_URL,
                                           timeout=10).json())
        logging.info(f"Dados brutos da API de tipos de tempo: {data}")
        weather_types.clear()
        for item in data["data"]:
            wid = item["idWeatherType"]
            desc = item.get("descWeatherTypePT",
...
def get_cities():
    ...
    if d:
        if d in district_mapping:
            return jsonify({"district_id": d,
                           "cities": district_mapping[d]["cities"]}), 200
...
def previsao():
    ...
    key = f"forecast_{gid}"
    try:
        data = cache.setdefault(key, requests.get(FORECAST_URL.format(id=gid),
                                                timeout=10).json())
        logging.info(f"Dados brutos da API de previsão para global_id {gid}: "
                     f"{data}")
        ...
        for f in data["data"]:
            ...
            if wid not in weather_types:
                logging.warning(f"Tipo de tempo {wid} não encontrado em "
                               f"weather_types")
            resp.append({
                "data": f["forecastDate"],
                "cidade": city_mapping[gid],
                "previsao": weather_types.get(wid,
                                             "Descrição não disponível no momento"),
