    ...
    if d:
        if d in district_mapping:
            return (
                jsonify({
                    "district_id": d,
                    "cities": district_mapping[d]["cities"]
                }),
                200
            )
...
def previsao():
    ...
    try:
        data = cache.setdefault(
            key,
            requests.get(
                FORECAST_URL.format(id=gid), timeout=10
            ).json()
        )
        logging.info(
            f"Dados brutos da API de previsão para global_id {gid}: {data}"
        )
        ...
        for f in data["data"]:
            ...
            if wid not in weather_types:
                logging.warning(
                    f"Tipo de tempo {wid} não encontrado em weather_types"
                )
            resp.append({
                "data": f["forecastDate"],
        ...
        return (
            jsonify({"previsoes": resp, "updated": data.get("dataUpdate")}),
            200
        )
    except Exception as e:
        logging.error(
            f"Erro ao buscar previsão para global_id {gid}: {e}"
        )
